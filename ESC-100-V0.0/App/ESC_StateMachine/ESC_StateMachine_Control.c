...
//===================================================================================
/// @brief  This function is control  process
/// @param  None
/// @return None
//===================================================================================
...
//			#endif
				// Update timer CCR value for required duty cycle
//      motor_ptr->temp_pwm_duty = 100;
      motor_ptr->pwm_control_duty_cycle = motor_ptr->pwm_duty_command;
      pwm_duty_control();
//      get_hall_value_and_change_mos_state();
      ESC_Commutator_on_Hall_Observer();
      set_duty(motor_ptr->temp_pwm_duty);

//		  if(1)//(((u32)feedback_Test2 | ((u32)feedback_Test3<<16)) == MIN_SPEED) && (motor_ptr->motorspeed != MIN_SPEED))
//      {				
...
//    get_hall_value_and_change_mos_state();
  ...
  if(control_machine_state_pre != control_machine_state)
  {
    SET_STDSCI_PACK_TX_OVERRUN(&(btx_ptr->pck_0x81), control_machine_state);
  }
	
  control_machine_state_pre = control_machine_state;
}
