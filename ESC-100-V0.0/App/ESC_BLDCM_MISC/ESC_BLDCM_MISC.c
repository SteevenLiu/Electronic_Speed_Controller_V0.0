...
/*PAC5XXX_RAMFUNC*/ void pwm_duty_control(void)
{
	...
	if(control_machine_state == STATE_CONTROL_COASTING)
  {
		if(((motor_ptr->pwm_duty_Fall_buf++) << 2) > motor_ptr->pwm_control_duty_change_delay_counts)
		{
			motor_ptr->final_pwm_duty = motor_ptr->pwm_control_duty_cycle;
		...
		} /*if(((motor_ptr->pwm_duty_Fall_buf++) << 1) > motor_ptr->pwm_control_duty_change_delay_counts)*/
  }
  else
  {
		if(motor_ptr->pwm_duty_Fall_buf++ > motor_ptr->pwm_control_duty_change_delay_counts)
		{
			motor_ptr->final_pwm_duty = motor_ptr->pwm_control_duty_cycle;
			if(motor_ptr->temp_pwm_duty < motor_ptr->final_pwm_duty)
			{
...
//
   void ESC_SPEED_PID(void)
   {
     ...
     motor_ptr->w_FeedBack = (0x310BE88<<4)/hall_ptr->dt_commutate_m_average;
     motor_ptr->w_Target = (motor_ptr->app_speed_command)<<4;

     motor_ptr->error_w = motor_ptr->w_Target - motor_ptr->w_FeedBack;
		 motor_ptr->s_error_w += motor_ptr->error_w;
//     if (motor_ptr->s_error_dt > motor_ptr->max_s_error_dt) motor_ptr->s_error_dt = motor_ptr->max_s_error_dt;
//     else if (motor_ptr->s_error_dt < (-motor_ptr->max_s_error_dt)) motor_ptr->s_error_dt = (-motor_ptr->max_s_error_dt); 
     if (motor_ptr->s_error_w > 0x1E000) motor_ptr->s_error_w = 0x1E000;
     else if (motor_ptr->s_error_w < (-0x1E000)) motor_ptr->s_error_w = -0x1E000;

     motor_ptr->p_dt = motor_ptr->error_w >> 10;
     motor_ptr->i_dt = motor_ptr->s_error_w >>10;

//     feedback_Test0 = motor_ptr->p_dt + motor_ptr->i_dt;
//     motor_ptr->pwm_control_duty_cycle = motor_ptr->p_dt + motor_ptr->i_dt;
	 }

   void ESC_SPEED_PID_Reset(void)
   {
     BLDCM_Controller* motor_ptr;
     motor_ptr = &bldcm_m1;
     motor_ptr->s_error_w = 0;
   }
...
void ESC_PWM_Cntr_Init(void)
{
  ...
  motor_ptr->motorspeed_hall_C_pre = motor_ptr->motorspeed_hall_C_current;
	...
	motor_ptr->motorspeed_cntr = MIN_SPEED_CNTR;
	motor_ptr->motorspeed =(motor_ptr->motorspeed_cntr) << LS_MotorSpeed_Cntr;//MIN_SPEED;
	motor_ptr->average_speed = motor_ptr->motorspeed;//MIN_SPEED;
	motor_ptr->app_measured_speed = motor_ptr->average_speed;//MIN_SPEED;
	motor_ptr->motorspeed_sum = (motor_ptr->average_speed)*NUM_SPEED_SAMPLES;//MIN_SPEED_SUM;

	for (i=0;i<NUM_SPEED_SAMPLES;i++)
  {
    motor_ptr->motorspeed_buffer[i] = motor_ptr->average_speed;//MIN_SPEED;
  }	
...
{
//	Sim_Detect* sim_ptr;
//	sim_ptr = &bldcm_sim_m1;

	BLDCM_Controller* motor_ptr;
	motor_ptr = &bldcm_m1;

  motor_ptr->control_mode_select_number	= 0; // directly control speed loop
//  motor_ptr->control_mode_select_number	= 2; // directly control PWM duty
...
//	motor_ptr->motorspeed_cntr = 65535;
...
//	tmp = fix16_mul_new_16_16(6 << 16, Hertz);
